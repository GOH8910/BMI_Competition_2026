...
function modelParameters = positionEstimatorTraining10(training_data)

    [nTrials, nDirs] = size(training_data);
    nNeurons = size(training_data(1,1).spikes, 1);
    wLen = 40;   % 40 ms non-overlapping windows

    modelParameters.nNeurons = nNeurons;
    modelParameters.nDirs    = nDirs;
    modelParameters.wLen     = wLen;

    % Find how many complete 40 ms windows fit in the shortest trial
    minLen = inf;
    for k = 1:nDirs
        for n = 1:nTrials
    ...
    % Feature: sqrt(spike count) per neuron in each 40 ms window
    ...
    for k = 1:nDirs
        ...
        for n = 1:nTrials
            ...
            for ti = 1:length(times)
                t   = times(ti);
                t_s = max(1, t - 39);   % current 40ms window
                fc  = sqrt(sum(sp(:, t_s:t), 2))';
                fcu = sqrt(sum(sp(:, 1:t), 2))' / sqrt(t / 40);
    ...
    modelParameters.smoothGammaEarly = 0.05;   % gamma at nAvailWin=8 (early)
    ...
    modelParameters.kinematicBlendEarly = 0.22;   % kinematic weight at nAvailWin=8 (early)
    ...
    modelParameters.dispWeightEarly  = 0.20;   % dispWeight at nAvailWin=8 (early)
