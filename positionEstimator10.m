...
function [x, y, newModelParameters] = positionEstimator10(test_data, modelParameters)
    ...
    temp  = tempEarly + (tempLate - tempEarly) * (nAvailWin - 8) / max(maxNWin - 8, 1);
    temp  = max(temp, 0.5);
    logP  = logP / temp;
    maxLP = max(logP);
    P     = exp(logP - maxLP);
    ...
    fi  = testFeat(1 : 4*nN);   % first 4 windows x 40ms = 160ms
    ...
    frac_disp  = (nAvailWin - 8) / max(maxNWin - 8, 1);
    ...
    if ~isempty(test_data.decodedHandPos)
        ...
        if nPrev >= 2
            ...
            frac_kin = (nAvailWin - 8) / max(maxNWin - 8, 1);
        ...
        frac     = (nAvailWin - 8) / max(maxNWin - 8, 1);
