function modelParameters = positionEstimatorTraining8(training_data)
    ...
    % Find shortest and longest trial lengths (used for maxNWin and preallocation)
    minLen = inf;
    maxLen_global = 0;
    for k = 1:nDirs
        for n = 1:nTrials
            L = size(training_data(n,k).spikes, 2);
            minLen = min(minLen, L);
            maxLen_global = max(maxLen_global, L);
        end
    end
    ...
    ridgeLambda = 650;
    nInitWin    = 4;
    nRidgeFeat  = nInitWin*nNeurons + nNeurons + nNeurons + 1 + 2;  % fi + fc + fcu + t/800 + startP
    nRowsMax    = nTrials * length(320:20:maxLen_global);            % upper bound for preallocation

    for k = 1:nDirs
        % Preallocate X and Y to avoid repeated array growth (AGROW)
        X   = zeros(nRowsMax, nRidgeFeat);
        Y   = zeros(nRowsMax, 2);
        row = 0;
        ...
        for n = 1:nTrials
            ...
            % Precompute cumulative spike sum once per trial — reused for every eval time
            % fcu at time t = sqrt(cumSpikes(:,t)) / sqrt(t/80); O(1) lookup vs O(t) sum
            cumSpikes = cumsum(sp, 2);   % nNeurons x Tmax

            for ti = 1:length(times)
                t   = times(ti);
                t_s = max(1, t - 79);
                fc  = sqrt(sum(sp(:, t_s:t), 2))';
                fcu = sqrt(cumSpikes(:, t))' / sqrt(t / 80);   % cumsum lookup — no recomputation
                row = row + 1;
                X(row, :) = [fi, fc, fcu, t/800, startP];
                Y(row, :) = hp(:, min(t, size(hp,2)))';
            end
        end
    ...
    % Preallocate: upper bound = nDirs * nTrials * max steps per trial
    maxStates    = nDirs * nTrials * length(320:20:maxLen_global);
    stateSeqCur  = zeros(4, maxStates);
    stateSeqNext = zeros(4, maxStates);
    stateIdx     = 0;
    ...
    for k = 1:nDirs
        for n = 1:nTrials
            ...
            for ei = 1:length(evalT)-1
                ...
                stateIdx = stateIdx + 1;
                stateSeqCur(:,  stateIdx) = [hp(1,t1); hp(2,t1); vx1; vy1];
                stateSeqNext(:, stateIdx) = [hp(1,t2); hp(2,t2); vx2; vy2];
            end
        end
    end

    % Trim to actual number of state pairs collected
    stateSeqCur  = stateSeqCur(:,  1:stateIdx);
    stateSeqNext = stateSeqNext(:, 1:stateIdx);
