function [x, y, newModelParameters] = positionEstimator8(test_data, modelParameters)
    ...
    logP = zeros(1, nDirs);
    for w = 1:nAvailWin
        feat  = winFeats(:, w)';   % 1 x nN
        SwInv = modelParameters.winLDA(w).SwInv;
        % Vectorised Mahalanobis: compute d*SwInv*d' for all 8 directions at once.
        % D(k,:) = feat - dirMean(k,:); diag(D*SwInv*D') = sum((D*SwInv).*D, 2)
        D    = modelParameters.winLDA(w).dirMeans - feat;   % nDirs x nN
        tmp  = D * SwInv;                                    % nDirs x nN
        logP = logP - 0.5 * sum(tmp .* D, 2)';              % 1 x nDirs
    end
